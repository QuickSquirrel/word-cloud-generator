                ...
                stage ("Test and install on stading") {
                    steps {
                        sh '''
                              ...
                              sudo curl -u downloader:123 -X GET "http://192.168.56.20:8081/repository/world-cloud-build/origin$git_branch/world-cloud-generator/1.$BUILD_NUMBER/world-cloud-generator-1.$BUILD_NUMBER.gz" -o /opt/wordcloud/word-cloud-generator.gz"
                              if [[ $? -ne 0 ]];
                                then
                                    echo "File not found"
                                    exit 1
