                ...
                stage ("Test and install on stading") {
                    steps {
                        sh '''
                              ...
                              sudo wget --http-user=downloader --http-password=123 -O /opt/wordcloud/word-cloud-generator.gz http://192.168.56.20:8081/repository/world-cloud-build/origin$git_branch/world-cloud-generator/1.$BUILD_NUMBER/world-cloud-generator-1.$BUILD_NUMBER.gz"
                              if [[ $? -ne 0 ]];
                                then
                                    echo "File not found"
                                    exit 1
