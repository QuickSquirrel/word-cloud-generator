pipeline {
    ...
    stages {
        ...
        stage ("Install on Stading") {
            steps {
                '''
                   sshpass -p 'vagrant' ssh vagrant@192.168.56.30 -o StrictHostKeyChecking=no "cd /opt/wordcloud/
                   sudo curl -u downloader:123 -X GET "http://192.168.56.20:8081/repository/word-cloud-build/$git_branch/world-cloud-generator/1.$BUILD_NUMBER/world-cloud-generator-1.$BUILD_NUMBER.gz" -o /opt/wordcloud/word-cloud-generator.gz
        ...
        stage ("Paralleled test") {
            parallel {
                stage ("Test on stading") {
                    steps {
                        sh '''
                              sshpass -p 'vagrant' ssh vagrant@192.168.56.30 -o StrictHostKeyChecking=no "cd /opt/wordcloud/
                              sudo curl -u downloader:123 -X GET "http://192.168.56.30:8081/repository/word-cloud-build/$git_branch/world-cloud-generator/1.$BUILD_NUMBER/world-cloud-generator-1.$BUILD_NUMBER.gz" -o /opt/wordcloud/word-cloud-generator.gz                                   
                              if [[ $? -ne 0 ]]
                                then
                                    echo "File not found"
                                    exit 1
